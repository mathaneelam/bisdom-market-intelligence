...
async def main():
    ...
    async with models_base.AsyncSessionLocal() as session:
        ...
        logger.info(f"Found Brief for date: {brief.brief_date}. Sending via Email...")
        
        email_sender = EmailSender()
        success = await email_sender.send_brief(brief)
        
        if success:
            logger.info("SUCCESS: The brief was successfully delivered to your Email!")
            # Mark as delivered
            brief.delivered_email = True
            await session.commit()
        else:
            logger.error("FAILED: Could not send the brief. Check your RESEND_API_KEY and TARGET_EMAILS in .env")
